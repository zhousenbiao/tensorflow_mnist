...
batch_size = 64   # 每一次64张图片进行训练
display_step = 20  # 每个20个记录打印一次

# 定义网络参数
n_input = 784   # 输入的维度（img shape:28*28）
n_classes = 10 # 标签的维度(0-9 digits)
...
def conv2d(name, l_input, w, b):
    x = tf.nn.conv2d(l_input, w, strides=[1, 1, 1, 1], padding='SAME')
